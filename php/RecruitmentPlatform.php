...
class RecruitmentPlatform {
    ...
    function employers_submit($company_name, $ad_title, $job_description, $contract_type, $category, $location, $address_line_1, $address_line_2, $address_city, $address_county, $address_postcode, $address_country, $website, $name, $email, $telephone) {
        require_once('db.php');
        $user_id = $_SESSION['user_id'];

        $STH = $DBH->prepare("INSERT INTO employers (company_name, ad_title, job_description, contract_type, category, location, address_line_1, address_line_2, address_city, address_county, address_postcode, address_country, website, name, email, telephone, user_id) value (:company_name, :ad_title, :job_description, :contract_type, :category, :location, :address_line_1, :address_line_2, :address_city, :address_county, :address_postcode, :address_country, :website, :name, :email, :telephone, :user_id)");
        $STH->bindParam(':company_name', $company_name);
        $STH->bindParam(':ad_title', $ad_title);
        $STH->bindParam(':job_description', $job_description);
        $STH->bindParam(':contract_type', $contract_type);
        $STH->bindParam(':category', $category);
        $STH->bindParam(':location', $location);
        $STH->bindParam(':address_line_1', $address_line_1);
        $STH->bindParam(':address_line_2', $address_line_2);
